...
function loadCurrentFlight() {
  ...
  .done(function (response) {
    ...
    console.log("Dep: " + flightObj.uDepTime);
      ...
      if (Object.prototype.toString.call(results[i].operationalTimes.estimatedGateArrival) === '[object Object]') {
        console.log("Y");
      }
      else {
        console.log("N");
      ...
      var arrDelay = moment(flightObj.uArrTime).diff(moment(oArrTime), "minutes");
      ...
      if (flightObj.uArrTime === "N/A") {
        ...
        var depTimeP = $("<p>").text("Updated departure time: No updates | Original departure time: " + moment(oDepTime).format("h:mm A on ddd, MMM Do"));
        var arrTimeP = $("<p>").text("Updated arrival time: No updates | Original arrival time: " + moment(oArrTime).format("h:mm A on ddd, MMM Do"));
      }
      else {
        var statusH = $("<h3>").text("Flight status: " + statusDesc);
        var depTimeP = $("<p>").text("Updated departure time: " + moment(flightObj.uDepTime).format("h:mm A on ddd, MMM Do") + " | Original departure time: " + moment(oDepTime).format("h:mm A on ddd, MMM Do"));
        var arrTimeP = $("<p>").text("Updated arrival time: " + moment(flightObj.uArrTime).format("h:mm A on ddd, MMM Do") + " | Original arrival time: " + moment(oArrTime).format("h:mm A on ddd, MMM Do"));
      };
