...
var amadeusLink = "https://api.sandbox.amadeus.com/v1.2/hotels/search-airport?apikey="+amadeusAuth
+"&location=sfo&check_in=2017-03-23&check_out=2017-03-24&amenity=RESTAURANT&amenity=INTERNET_PUBLIC_AREAS&number_of_results=5";
...
=======
	...
	for (var i = 0; i < hotels.length; i++){
		...
		var myObject = {
			name: hotels[i].property_name || "N/A",
			address: hotels[i].address.line1 || "N/A",
			city: hotels[i].address.city || "N/A",
			state: hotels[i].address.region || "N/A",
			zip: hotels[i].address.postal_code || "N/A",
			country: hotels[i].address.country || "N/A",
			price: hotels[i].total_price.amount || "N/A",
			amenities: (Array.isArray(hotels[i].amenities) && hotels[i].amenities.length > 0 && hotels[i].amenities[1].description) || "N/A",
			description: (Array.isArray(hotels[i].rooms) && hotels[i].rooms.length > 0 && hotels[i].rooms[0].descriptions) || "N/A",
			phone: (Array.isArray(hotels[i].contacts) && hotels[i].contacts.length > 0 && hotels[i].contacts[0].detail) || "N/A",
