...
var checkInDate = new Date(), checkOutDate = new Date();
var departureAirport = $("#departureAirport"), amadeusAuth = "TppXrVyzrRIqOWzvMyPsAO253AZe9mhK";

checkInDate = moment(checkInDate).format("YYYY-MM-DD");
checkOutDate = moment().add(2, "days").format("YYYY-MM-DD");


var amadeusLink = "https://api.sandbox.amadeus.com/v1.2/hotels/search-airport?apikey="+amadeusAuth+"&location="+departureAirport+"&check_in="
  ...
  db.ref().on("child_added", function(childSnapshot){
	$.ajax({
		...
		for (var i = 0; i < hotels.length; i++){
			...
			$("#textArea")
			.appendChild(myObject.name+"<br>"+myObject.address+"<br>"+myObject.city+", "+myObject.state+", "+myObject.zip+"<br>"+myObject.country+"<br><span><strong>The price of stay: $"+
